...
fn main() -> io::Result<()> {
    // Memory-map the file
    let file = File::open("/home/samhaug/dl/CAPELLA_C03_SM_CPHD_HH_20211229053627_20211229053631.cphd")?;
...
//    println!("header as utf 8: {:?}", header_str);
    ...
    println!("  Support Block Size: {:?} bytes", header.support_block_size);
    println!("  Support Block Offset: {:?} bytes", header.support_block_byte_offset);
    ...
    println!("  Signal Block Offset: {} bytes", header.signal_block_byte_offset);
